...
import org.breezyweather.sources.cwa.json.CwaUvResult
...
fun convertSecondary(
    ...
    return SecondaryWeatherWrapper(
        airQuality = if (airQualityResult != null) AirQualityWrapper(current = getAirQuality(airQualityResult)) else null,
...
private fun getCurrent(
    ...
    // If the UV Index station is under maintenance, the API call will report -99.
    // We should disregard those reports.
    if (uvResult.records!!.Station?.get(0)!!.WeatherElement.UVIndex >= 0) {
        uV = UV(
            index = uvResult.records!!.Station?.get(0)!!.WeatherElement.UVIndex
        )
    }
...
// Concentrations of SO₂, NO₂, O₃ are given in ppb (and in ppm for CO).
// We need to convert these figures to µg/m³ (and mg/m³ for CO).
private fun ppbToUgm3(pollutant: String, ppb: Double?): Double? {
    ...
    // assume 25C
    return ppb / 0.0820573660809596 / 298.15 * molecularMass
}